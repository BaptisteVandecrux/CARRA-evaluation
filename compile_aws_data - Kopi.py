...
path_l3 = '../PROMICE data/thredds-data/level_3_sites/'
...
freq = 'D'
freq_tag = 'hour' if freq =='H' else 'day'
# %% loading PROMICE stations
print('loading PROMICE')
df_meta_promice = pd.read_csv(path_l3+'../metadata/AWS_sites_metadata.csv')
df_meta_promice = df_meta_promice.loc[df_meta_promice.location_type == 'ice sheet']
df_meta_promice['source'] ='PROMICE/GC-Net'

for station in df_meta_promice.site_id:    
    print('   ',station)
    df_aws = pd.read_csv(f'{path_l3}/csv/{freq_tag}/{station}_{freq_tag}.csv')
...
for station in ['CP2', 'JAR2', 'JAR3',  'Summit', 'NGRIP','KAR', 'KULU', 'Aurora', 'Petermann Glacier',
       'Petermann ELA', 'SMS-PET', 'SMS1', 'SMS2', 'SMS3', 'SMS4', 'SMS5']:
    print('   ',station)
