...
good_stations = ds_aws.stid.values[~ds_aws.stid.isin(unwanted)]
...
# for var in ['dsr','usr','dlr','ulr']:
    ...
    for station in ['KAN_U','DY2']:
        if station == 'CEN': main_station = 'CEN2'
        filename = f'data/CARRA_at_AWS/{station}.nc'
        if not os.path.exists(filename):
            filename = f'data/CARRA_at_AWS/{main_station}.nc'
        print(filename)
        
            
        if os.path.exists(filename):
            ds_carra = xr.open_dataset(filename)
        else:
        ...
        df_carra = ds_carra.isel(station=0).to_dataframe()
        df_carra['name'] = station
        ...
        if var == 'dsr':
            ...
            if len(common_idx)<100:
                ...
                continue
            df_carra_filled = df_carra.loc[common_idx].ffill()
            df_aws_filled = df_aws.loc[common_idx].ffill()
        ...
        tmp['elevation_CARRA'] =  ds_carra.altitude_mod.item()
